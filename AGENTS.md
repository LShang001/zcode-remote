...
# --notes 就是 App 更新弹窗给用户看的"更新说明",必须面向用户写(几个要点即可,别只写"修 bug")
git add -A && git commit -m "vX.Y: ..." && git push
cp app/build/outputs/apk/release/app-release.apk /tmp/ZCodeRemote-vX.Y.apk
gh release create vX.Y /tmp/ZCodeRemote-vX.Y.apk --title "vX.Y" --notes "本版要点:
- 修复了什么
- 新增了什么"
```
...
3. **发版四同步** — `versionCode` +1、`versionName`、GitHub Release(tag `vX.Y` + 上传 APK + **必写更新说明**),一次发版四项同改;界面显示的版本号运行时读 PackageInfo,无需人肉同步;漏发 Release 等于用户永远收不到更新。**升级功能验证通过后主动发版,不用问**(来源:2026-09-08 用户指定"以后升级验证好了就主动发版";此前 2026-09-04~09-07 的"等用户说发布"规则作废)
4. **更新说明是功能的一部分**(2026-09-14 用户指定"每次升级要有更新说明")— Release notes 不只写在 GitHub 上,App 的"发现新版本"弹窗会**直接读取并展示** Release notes 正文(v2.10 起):所以 notes 必须用面向用户的普通中文写(不要写 commit/文件名/内部代号),App 端只做轻度 markdown 清理(`#`/`**`/反引号);notes 为空时弹窗退化为只显示下载提示。**发版时 notes 至少要有一条"这版改了什么"**,否则用户看到的更新弹窗没有说明
